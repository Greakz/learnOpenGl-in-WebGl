...
out vec4 vDirLightSpaceFragPos;
out vec4 vPointLightSpaceFragPos;
...
void main(void) {
    ...
    vDirLightSpaceFragPos = dir_light_space_matrix * vec4(vSurfacePosition, 1.0);
}
...
in vec4 vDirLightSpaceFragPos;

const float depth_buffer_reference_bias = 0.005;
const float calculated_depth_reference_bias = 0.0032;
...
float calculatePointShadow() {
    vec3 fragToLight = vSurfacePosition - point_position;
    float closestDepth = texture(point_shadow_map, fragToLight).r;
    // closestDepth *= (point_shadow_far_plane - 1.0);
    float currentDepth = length(fragToLight) / point_shadow_far_plane;
    /*
    if(currentDepth > 1.0) {
        return 1.0;
    }
    */
    float shadow = currentDepth + (calculated_depth_reference_bias) < closestDepth ? 1.0 : 0.0;
    //return shadow;
    return shadow;
}
...
float calculateDirectionalShadow() {
    ...
    vec3 projCoords = vDirLightSpaceFragPos.xyz / vDirLightSpaceFragPos.w;
    ...
    for(int x = -1; x <= 1; ++x)
    {
        for(int y = -1; y <= 1; ++y)
        {
            ...
            shadow += currentDepth - depth_buffer_reference_bias > pcfDepth ? add : 0.0;
        }
    }
    shadow /= 11.0;
...
void main(void) {
    ...
    */
    ...
    float point_shadow = calculatePointShadow();

    // Combine all results
    vec3 amb_light_result =  /*dir_amb_light_res + */ point_amb_light_res /* + spot_amb_light_res*/;
    vec3 diff_light_result = /*(vec3(dir_shadow) * dir_diff_light_res) +*/ vec3(point_shadow) * point_diff_light_res/* + spot_diff_light_res*/;
    vec3 spec_light_result = /*(vec3(dir_shadow) * dir_spec_light_res) +*/ vec3(point_shadow) * point_spec_light_res/* + spot_spec_light_res*/;
